...
public class mongodbFunc {
    ...
    public String show_friend_list(String token) {
        String friend_list = null;
        System.out.println("input: " + token);
        if (check_value(auth, "token", token) == true) {
            BasicDBObject whereQuery = new BasicDBObject();
            whereQuery.put("token", token);
            DBCursor cursor = auth.find(whereQuery);
            String username = null;
            ...
            if (check_value(users, "user", username) == true) {
                ...
                whereQuery2.put("user", username);
                DBCursor cursor2 = flist.find(whereQuery2);
                Object test = new Object();
                while (cursor2.hasNext()) {
                    cursor2.next();
                    //System.out.println(cursor2.next());
                    friend_list = cursor2.curr().get("friends").toString();
                }
